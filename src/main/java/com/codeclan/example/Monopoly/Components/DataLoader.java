...
public class DataLoader  implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args){
        ...
        Squares go = new Squares(0,"Go",null, 0, 0, 0, 0, 0,0,0, 0);
        squaresRepository.save(go);

        Squares okr = new Squares(1,"Old Kent Road", "brown",60, 2, 10, 30, 90,160,250, 50);
        squaresRepository.save(okr);

        Squares cc1 = new Squares(2, "Community Chest", "communitychest",0, 0, 0, 0, 0, 0, 0, 0);
        ...
        Squares tai = new Squares(6, "The Angel Islington", "lightblue", 100, 6, 30, 90,270,400,550,50);
        squaresRepository.save(tai);

        Squares c1 = new Squares(7, "Chance", "chance", 0, 0,0 ,0 ,0 ,0 ,0 , 0);
        squaresRepository.save(c1);

        Squares er = new Squares(8, "Euston Road", "lightblue", 100,6, 30,90,270,400,550,50);
        squaresRepository.save(er);

        Squares pv = new Squares(9, "Pentonville Road", "lightblue", 120, 8, 40, 100, 300, 450, 600, 50);
        squaresRepository.save(pv);

        Squares jail = new Squares( 10, "Jail", null, 0,0,0,0,0,0,0,0);
        squaresRepository.save(jail);

        Squares pm = new Squares(11, "Pall Mall", "deeppink", 140, 10, 50, 150, 450, 625, 750, 100 );
        squaresRepository.save(pm);

        Squares ec = new Squares(12, "Electric Company", "Utility", 150,0, 0, 0, 0, 0, 0, 0 );
        squaresRepository.save(ec);

        Squares w = new Squares(13, "Whitehall", "deeppink", 140, 10,50,150,450,625, 750, 100);
        ...
        Squares vs = new Squares(19, "Vine Street", "orange",200, 16, 80, 220, 600, 800, 1000, 100);
        squaresRepository.save(vs);

        Squares free = new Squares(20, "Free Parking", null, 0, 0, 0, 0, 0, 0, 0, 0);
        squaresRepository.save(free);

        Squares s = new Squares(21, "Strand", "red", 220, 18,90, 250,700, 875, 1050, 150 );
        squaresRepository.save(s);

        Squares c2 = new Squares(22, "Chance", "chance", 0, 0, 0, 0, 0, 0, 0, 0);
        squaresRepository.save(c2);

        Squares fs = new Squares(23, "Fleet Street", "red", 220, 18, 90, 250, 700, 875, 1050, 150);
        squaresRepository.save(fs);

        Squares ts = new Squares(24, "Trafalgar Square", "red", 240, 20, 100,300,750,925, 1100, 150);
        squaresRepository.save(ts);

        Squares fss = new Squares(25, "Fenchurch St. Station", "station", 200, 0, 0, 0, 0, 0, 0, 0);
        squaresRepository.save(fss);

        Squares ls = new Squares(26, "Leicester Square", "yellow", 260, 22, 110, 330,800,975,1150, 150 );
        squaresRepository.save(ls);

        Squares cs = new Squares(27, "Coventry Street", "yellow", 260, 22, 110, 330, 800, 975, 1150, 150);
        squaresRepository.save(cs);

        Squares ww = new Squares(28, "Water Works", "utility", 150, 0, 0, 0, 0, 0, 0, 0 );
        squaresRepository.save(ww);

        Squares p = new Squares(29, "Piccadilly", "yellow", 280, 24, 120, 360, 850, 1025, 1200, 150 );
        squaresRepository.save(p);

        Squares gtj = new Squares(30, "Go to Jail", null, 0,0,0,0,0, 0, 0, 0);
        squaresRepository.save(gtj);

        Squares rs = new Squares(31, "Regent Street", "green", 300, 26, 130, 390, 900, 1100, 1275, 200 );
        squaresRepository.save(rs);

        Squares os = new Squares(32, "Oxford Street", "green", 300, 26, 130, 390, 900, 1100, 1275, 200);
        squaresRepository.save(os);

        Squares cc3 = new Squares(33, "Community Chest", "communitychest", 0, 0, 0, 0, 0, 0, 0, 0 );
